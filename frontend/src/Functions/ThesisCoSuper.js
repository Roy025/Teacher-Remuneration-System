...
const ThesisCoSuper = ({ inputFields, setInputFields, field}) => {
  const removeInputFields = (e, index) => {
    e.preventDefault();
    const rows = [...inputFields];
    rows[0].cosuper.splice(index, 1);
    setInputFields(rows);
  };

  console.log(inputFields[0].cosuper);

  const handleChangeName = (evnt, index) => {
    const { name, value } = evnt.target;
    const list = [...inputFields];
    list[0].cosuper[index].name = value;
    setInputFields(list);
  };
  ...
  const handleChangeStudent = (evnt, index) => {
    const { name, value } = evnt.target;
    const list = [...inputFields];
    list[0].cosuper[index].stdntID = value;
    setInputFields(list);
  };
  ...
  return (
    <div className="Container">
      {inputFields[0].cosuper.map((data, index) => {
        const { name, id } = data;
        return (
          <div className="ParentFormRow" key={index}>
            <div
              className={
                inputFields[0].cosuper.length === 1 ? "FormRow" : "FormRow CrossFormRow"
              }
              key={index}
            >
              ...
              {inputFields[0].cosuper.length !== 1 ? (
            ...
            {inputFields[0].cosuper.length - 1 === index && (
              <div className="FormRowElement">
                <button
                  className="addButton"
