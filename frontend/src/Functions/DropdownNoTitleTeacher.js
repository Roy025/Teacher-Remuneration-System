...
const DropdownNoTitleTeacher = ({
  ...
  const handleChange = (evnt) => {
    const arr = options.filter((x) => {
      if (x.name)
        return x.name.toLowerCase().includes(evnt.target.value.toLowerCase());
      else
        return x.code.toLowerCase().includes(evnt.target.value.toLowerCase());
    });
    // console.log("arr", arr);
