...
public class ExamForDirectorSpecification : BaseSpecification<Exam>
{
    ...
    )
    {
        ...
        AddInclude(x => x.Include(x => x.LabCourses).ThenInclude(c => c.Course));
        AddInclude(x => x.Include(x => x.TheoryCourses).ThenInclude(c => c.Course));
        AddInclude(x => x.Include(x => x.TermPapers).ThenInclude(c => c.Course));
    }
}
